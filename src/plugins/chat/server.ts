...
proxy.server.chatMessage = (player, msg) => {
  if (msg[0] === '/') {
    ...
    if (msg.length > 0) {
      ...
      proxy.client.chatMessage(player.name, msg.replace(/</g, '&lt;').replace(/'/g, '&#39').replace(/"/g, '&#34'));
    }
  }
};

export const send = (player: alt.Player, msg: string): void => {
  proxy.client.chatMessage(player, null, msg);
};

export const broadcast = (msg: string): void => {
  proxy.client.chatMessage(null, msg);
};

export const mutePlayer = (player: alt.Player, state: boolean): void => {
