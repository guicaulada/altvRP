...
proxy.local.registerCommand('pos', (player: alt.Player) => {
  proxy.client.chatMessage(player, null, `${player.pos.x}, ${player.pos.y}, ${player.pos.z}`);
});

proxy.local.registerCommand('freemode', (player: alt.Player, gender: string) => {
  player.model = `mp_${gender}_freemode_01`;
});

proxy.local.registerCommand('tp', (player: alt.Player, x: string, y: string, z: string) => {
